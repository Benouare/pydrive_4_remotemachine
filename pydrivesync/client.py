...
class PyDriveSync():
    ...
    def list_all_files_google(self,id="root",name=None):
        files = self.drive.ListFile({'q': "'{}' in parents and trashed=false".format(id)}).GetList()
        for file in files:
            if not id == "root":
                file["title"] = os.path.join(name,file["title"])
            if file["mimeType"] == "application/vnd.google-apps.folder":
                self.list_all_files_google("{}".format(file["id"]),file["title"])
    ...
    def __init__(self,file_path,file_config):
        self.download_folder = os.path.join(os.path.abspath(os.getcwd()),file_path,self.DRIVE_PATH)
        ...
        files_gdrive =[os.path.join(self.download_folder,x["title"]) for x in self.current_remote_files]
        ...
        for file in self.current_remote_files:
           print("###")
           print("Processing remote file {} ({})".format(file['title'],file["mimeType"]))
           if file["mimeType"] == "application/vnd.google-apps.folder":
               if not os.path.exists(os.path.join(self.download_folder,file['title'])):
                   os.makedirs(os.path.join(self.download_folder,file['title']))
           if file["mimeType"] not in ["application/vnd.google-apps.document","application/vnd.google-apps.folder"]:
               ...
               if os.path.join(self.download_folder,file['title']) in self.current_files.keys():
                   md5 = self.md5(os.path.join(self.download_folder,file['title']))
                   if md5 == file["md5Checksum"]:
                       must_download = False
                   print("{} alredy there, checking md5 are same? {} ('{}' vs '{}')".format(file['title'],not must_download,md5 ,file["md5Checksum"]))
               if must_download is True:
                   if not os.path.exists(os.path.dirname(os.path.join(self.download_folder,file['title']))):
                       os.makedirs(os.path.dirname(os.path.join(self.download_folder,file['title'])))
                   file.GetContentFile(os.path.join(self.download_folder,file['title']))
